...
async function applyPatch(diffPatch) {
    const answer = await vscode.window.showWarningMessage('Apply the proposed BeliefGuard patch to your workspace?', { modal: true }, 'Apply Patch');
    ...
    try {
        ...
        vscode.window.showErrorMessage(`BeliefGuard could not apply the patch: ${error?.message || 'Unknown error'}`);
    }
}
