...
function evaluateState(beliefs) {
    var hasUnvalidatedHighRisk = false;
    ...
    for (var _i = 0, beliefs_1 = beliefs; _i < beliefs_1.length; _i++) {
        ...
        if (belief.contradictions.length > 0) {
            var contradictsImmutable = belief.contradictions.some(function (cId) {
                var contradicting = beliefs.find(function (b) { return b.id === cId; });
...
function getBlockingContradictions(beliefs) {
    return beliefs.filter(function (b) { return b.contradictions.length > 0; });
}
